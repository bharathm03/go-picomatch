...
// Tests for the ECMAScript acceptance predicate. Nothing here states picomatch's
// answer for a pattern — testdata/tokens and testdata/emit hold those — and the
// one table that states a foreign semantic states JavaScript's, not picomatch's.
...
// encode is internal/parse's own conversion, repeated here rather than imported:
// the point of the package split is that this one does not depend on that one.
func encode(s string) units { return utf16.Encode([]rune(s)) }

// TestValidIsTheJavaScriptGrammar pins Valid against the
...
func TestValidIsTheJavaScriptGrammar(t *testing.T) {
	for _, tc := range []struct {
		...
		if got := Valid(encode(tc.src)); got != tc.want {
			t.Errorf("Valid(%q) = %v, want %v (JavaScript new RegExp(%q))",
	...
	if Valid(units{'[', 0xDE00, '-', 0xD83D, ']'}) {
		t.Error(`Valid("[\uDE00-\uD83D]") = true, want false — 0xDE00 > 0xD83D as code units`)
	}
	if !Valid(units{'[', 0xD83D, '-', 0xDE00, ']'}) {
